// Sydney Achievements Map – pin markers, no clustering, with fuzzy search,
...
const DEFAULT_ZOOM =
  (window.ACH_MAP_CONFIG && typeof ACH_MAP_CONFIG.zoom !== "undefined")
    ? ACH_MAP_CONFIG.zoom
    : 11.45;

const MIN_ZOOM =
  (window.ACH_MAP_CONFIG && typeof ACH_MAP_CONFIG.minZoom !== "undefined")
    ? ACH_MAP_CONFIG.minZoom
    : 10;

const MAX_ZOOM =
  (window.ACH_MAP_CONFIG && typeof ACH_MAP_CONFIG.maxZoom !== "undefined")
    ? ACH_MAP_CONFIG.maxZoom
    : 17;

const SCROLL_WHEEL =
  (window.ACH_MAP_CONFIG && typeof ACH_MAP_CONFIG.scrollWheelZoom !== "undefined")
    ? !!ACH_MAP_CONFIG.scrollWheelZoom
    : false;
...
// ---------------------------
const ROOT_EL = document.querySelector('.achievements-map-wrapper[data-ach-map-mode]');
const ACH_MAP_MODE = ROOT_EL ? ROOT_EL.getAttribute('data-ach-map-mode') : 'full';
const IS_SHAPE_ONLY = ACH_MAP_MODE === 'shape-only';
...
const achievementMarkers = L.layerGroup();
if (SHOW_MARKERS) {
  achievementMarkers.addTo(map);
}
...
// Data state
// ---------------------------
let achievementsx = [];           // normalised achievementsx
let categories = [];
let categoriesById = {};
let fuse = null;
...
// store { marker, achievement } by title
...
// Normalise raw achievement from JSON
function normaliseAchievement(raw) {
  if (!raw) return null;
  ...
  return {
    title: raw.title || "Untitled achievement",
    categoryId,
...
function buildFuseIndex() {
  ...
  fuse = new Fuse(achievementsx, options);
}

function applySearchAndCategory() {
  let baseAchievements = achievementsx;
  const q = searchQuery.trim();

  if (q && fuse) {
    const results = fuse.search(q);
    baseAchievements = results.map((r) => r.item);
  } else if (q) {
    const lower = q.toLowerCase();
    baseAchievements = achievementsx.filter((ev) =>
  ...
  const filtered = baseAchievements.filter((ev) =>
    activeCategoryId === ALL_CATEGORY_ID
      ? true
      : ev.categoryId === activeCategoryId
  );

  renderAchievementList(filtered);
  drawAchievementMarkers(filtered);
}
...
async function initBoundaries() {
  const regionPath =
    (window.ACH_MAP_CONFIG && ACH_MAP_CONFIG.regionUrl) ||
...
function clearActiveMarker() {
  if (activeMarkerEntry && activeMarkerEntry.marker) {
    const { marker, achievement } = activeMarkerEntry;
    marker.setIcon(createPinIcon(achievement, false));
  }
  activeMarkerEntry = null;
}

function highlightMarkerForAchievement(ev) {
  clearActiveMarker();
  const entry = markersByTitle.get(ev.title);
  if (entry && entry.marker) {
    entry.marker.setIcon(createPinIcon(entry.achievement, true));
    activeMarkerEntry = entry;
  }
}

function clearActiveAchievementCard() {
  document
    .querySelectorAll(".achievement.achievement--active")
    .forEach((el) => el.classList.remove("achievement--active"));
}

function highlightAchievementCard(ev) {
  clearActiveAchievementCard();
  const listEl = document.getElementById("achievement-list");
  if (!listEl) return;
  const card = listEl.querySelector(`[data-title="${ev.title}"]`);
  if (card) {
    card.classList.add("achievement--active");
...
// Render list & details
// ---------------------------
function renderAchievementList(achievementArray) {
  const listEl = document.getElementById("achievement-list");
  ...
  if (!achievementArray.length) {
    listEl.innerHTML =
      '<div class="text-muted" style="padding:8px 4px;">No achievements match your filters.</div>';
    return;
  }

  achievementArray.forEach((ev) => {
    ...
    const thumbHtml = ev.image
      ? `
        <div class="achievement-thumb skeleton">
          <img
            src="${ev.image}"
            alt="${ev.title}"
            class="achievement-thumb-img"
    ...
    card.innerHTML = `
      ...
      <div class="achievement-content">
        <p class="achievement-title mb-0">${ev.title}</p>
        <div class="achievement-meta">
          <span class="achievement-location">${ev.location}</span>
        </div>
      </div>
    `;

    card.addEventListener("click", () => {
      showAchievementDetails(ev);
      highlightMarkerForAchievement(ev);
      highlightAchievementCard(ev);
    });
...
function showAchievementDetails(ev) {
  const listEl = document.getElementById("achievement-list");
  const detailsEl = document.getElementById("achievement-details");
  if (!listEl || !detailsEl) return;
  ...
  const thumbHtml = ev.image
    ? `
      <div class="achievement-thumb skeleton" style="margin-bottom:10px;">
        <img
          src="${ev.image}"
          alt="${ev.title}"
          class="achievement-thumb-img"
  ...
  <a href="#" id="back-to-list" >&larr; Back to results</a>
    ...
    <p class="achievement-title">${ev.title}</p>
  ...
  if (backBtn) {
    backBtn.addEventListener("click", (e) => {
      ...
      clearActiveAchievementCard();           // optional: clear list highlight
    });
  }
...
// Draw markers (pins)
// ---------------------------
function drawAchievementMarkers(achievementArray) {
  if (!SHOW_MARKERS) return;
  achievementMarkers.clearLayers();
  ...
  achievementArray.forEach((ev) => {
    ...
    markersByTitle.set(ev.title, { marker, achievement: ev });
    achievementMarkers.addLayer(marker);
  });
}
...
function initCategoryTabs() {
  ...
  const usedCategoryIds = getUsedCategoryIds(achievementsx);
...
function initSearch() {
  const form = document.getElementById("achievement-search-form");
  const input = document.getElementById("achievement-search-input");
...
async function loadCategories() {
  try {
    const apiUrl =
      (window.ACH_MAP_CONFIG && ACH_MAP_CONFIG.categoriesApiUrl) ||
      (window.ACH_MAP_CONFIG && ACH_MAP_CONFIG.categoriesUrl) ||
      "categories.json";
...
async function loadAchievements() {
  try {
    ...
    const apiUrl =
      (window.ACH_MAP_CONFIG && ACH_MAP_CONFIG.achievementsApiUrl) ||
      (window.ACH_MAP_CONFIG && ACH_MAP_CONFIG.achievementsUrl) ||
      "entries.json";

    const res = await fetch(apiUrl, { cache: "no-cache" });
    if (!res.ok) throw new Error(`achievements HTTP ${res.status}`);

    const rawAchievements = await res.json();
    // If REST returns wrapped object, adjust here; assuming array:
    achievementsx = rawAchievements.map(normaliseAchievement).filter(Boolean);
  } catch (err) {
