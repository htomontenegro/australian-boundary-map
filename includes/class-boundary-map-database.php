<?php
/**
 * Database layer for Entries Map.
...
class Boundary_Map_Database
{
    ...
    private static function find_legacy_tables($required_columns)
    {
        global $wpdb;

        $pattern = $wpdb->esc_like($wpdb->prefix . 'ach_') . '%';
        $candidate_tables = $wpdb->get_col($wpdb->prepare('SHOW TABLES LIKE %s', $pattern));
        $matches = array();

        foreach ($candidate_tables as $candidate_table) {
            $columns = $wpdb->get_col("DESCRIBE {$candidate_table}", 0);
            if (empty(array_diff($required_columns, $columns))) {
                $matches[] = $candidate_table;
            }
        }
    ...
    private static function migrate_legacy_table($new_table_name, $legacy_table_names, $columns)
    {
        global $wpdb;

        foreach ($legacy_table_names as $legacy_table_name) {
            if ($legacy_table_name === $new_table_name || !self::table_exists($legacy_table_name)) {
                continue;
            }
            ...
            $wpdb->query("INSERT IGNORE INTO {$new_table_name} ({$column_list}) SELECT {$column_list} FROM {$legacy_table_name}");
    ...
    public static function migrate_legacy_tables()
    {
        self::migrate_legacy_table(
            ...
            self::find_legacy_tables(array('title', 'category_id', 'lat', 'lng', 'status')),
            array('id', 'title', 'category_id', 'category_label', 'description', 'location', 'lat', 'lng', 'image', 'status', 'created_at', 'updated_at')
        );

        self::migrate_legacy_table(
            self::get_categories_table(),
            self::find_legacy_tables(array('label', 'color', 'icon')),
            array('id', 'label', 'color', 'icon', 'sort_order')
        );
    }
    ...
    {
        global $wpdb;
        self::$entries_table = $wpdb->prefix . 'boundary_map_entries';
        self::$categories_table = $wpdb->prefix . 'boundary_map_categories';
    }
    ...
    public static function create_tables()
    {
        ...
        update_option('boundary_map_db_version', '1.0');
    }
    ...
    {
        return get_option('boundary_map_migrated_to_db', get_option('ach_map_migrated_to_db', false));
    }
    ...
    public static function migrate_from_json($entries_file, $categories_file)
    {
        ...
        if (get_option('boundary_map_migrated_to_db', false)) {
            return;
        }
        ...
        update_option('boundary_map_migrated_to_db', true);
    }
    ...
    /**
     * Count entries assigned to a category id.
     */
    public static function count_entries_for_category($category_id)
    {
        global $wpdb;
        $table = self::get_entries_table();

        return intval($wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $table WHERE category_id = %s",
            sanitize_text_field($category_id)
        )));
    }

    /**
     * Update stored category references on entries after a category edit.
     */
    public static function update_entry_category($old_category_id, $new_category_id, $new_category_label)
    {
        global $wpdb;
        $table = self::get_entries_table();
        ...
        return $wpdb->update(
            $table,
            array(
                'category_id' => sanitize_text_field($new_category_id),
                'category_label' => sanitize_text_field($new_category_label),
            ),
            array(
                'category_id' => sanitize_text_field($old_category_id),
            )
        );
    }
